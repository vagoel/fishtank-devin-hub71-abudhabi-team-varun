...
import { isActive, localTime, siteStatus, timeAgo } from './domain/incidents'
...
function App() {
  ...
  const latestNotice = dashboard.notices[0]
  ...
  return <div ref={page} className="control-room">
    ...
    <header className="app-header">
      ...
      <div className="header-right"><span className="demo-tag">{dashboard.mode === 'demo' ? 'DEMO ENVIRONMENT' : 'API CONNECTED MODE'}</span><div className="operator-avatar" title="Demo operator">CR</div><div className="operator-label"><strong>Control room</strong><small>Operations team</small></div></div>
    </header>
        ...
        <div className="operations-grid" id="incident-workspace" data-enter>
          <section className="map-panel" aria-label="Abu Dhabi map and site explorer">
            ...
            <div className="map-content"><MapBoundary><Suspense fallback={<div className="map-loading"><span className="loading-orbit" />Loading the city view</div>}><AbuDhabiMap sites={sites} incidents={incidents} dispatches={dispatches} selectedSiteId={selectedSiteId} stale={dashboard.stale} onSelectSite={selectSite} view={view} /></Suspense></MapBoundary>
              ...
              {!explorer && <button className="site-summary-card" onClick={() => { setExplorer('sites'); setSearch('') }}><span className="eyebrow">MONITORING FOOTPRINT</span><strong>{sites.length}<span>construction sites</span></strong><div className="site-health-track">{sites.map(site => <span key={site.id} className={siteStatus(site, incidents, dashboard.stale)} />)}</div><span className="site-summary-link">Explore locations <ArrowUpRight size={13} /></span></button>}
            </div>
            <div className="map-panel-footer"><span><Wifi size={12} />{dashboard.mode === 'demo' ? 'SIMULATED TELEMETRY' : 'API TELEMETRY'}</span><span>Last sync {dashboard.lastUpdated ? timeAgo(dashboard.lastUpdated, now) : 'pending'}<i />Human approval enabled</span></div>
          </section>
